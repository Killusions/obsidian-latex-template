...
class WandbDownloader:
    def __init__(self, entity, project, example_config_param, top_n_runs=5):
        self.entity = entity
        self.project = project
        self.top_n_runs = top_n_runs
        self.example_config_param = example_config_param
        self.api = wandb.Api()
        wandb.login()
    ...
    def get_best_runs(self, metric_name, metric_ascending=False):
        ...
        for run in runs:
            ...
            config = run.config
            matches = True
            if self.example_config_param is not None:
                actual_example_config_param = config.get("example_config_param")
                matches &= actual_example_config_param == self.example_config_param
    ...
    def get_and_collect_best_runs(self, metric_name, file_names, metric_ascending=False, run_id=None):
        ...
        if best_runs:
            print(f"{self.project}: Found {len(best_runs)} matching runs for param {self.actual_example_config_param}.")
        else:
            print(f"{self.project}: No matching runs found for param {self.actual_example_config_param}.")
